...
test("Par3 encodes recovery shards in place and marks them present", () => {
  const originalShards = buildOriginalShards(3, 32);
  ...
  try {
    ...
    assert.deepEqual(codec.encode(), [3, 4]);
...
test("Par3 encode rejects layouts with missing original shards", () => {
  ...
  try {
    ...
    assert.throws(() => codec.encode(), /original shard 1 must be written before encode/);
  } finally {
    codec.free();
  }
});

test("Par3 encode is a no-op when no recovery slots are configured", () => {
...
test("Par3 rejects impossible repairs before allocating a shard arena", () => {
  const codec = new Par3(Par3.resolveLayout({ originalCount: 2, recoveryCount: 1, shardSize: 4 }));

  try {
    assert.equal(codec.arenaHandle, 0n);
    assert.throws(() => codec.repair(), /Insufficient shards provided for repair/);
    assert.equal(codec.arenaHandle, 0n);
  } finally {
    codec.free();
  }
