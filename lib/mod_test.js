...
import * as par3RawBindings from "#pkg/par3_bg.js";
...
test("resolveWasmExports accepts instantiated and fallback module shapes", () => {
  const fakeExports = {
    __wbindgen_start() {},
  };

  assert.equal(resolveWasmExports(fakeExports), fakeExports);
  assert.equal(resolveWasmExports({ default: fakeExports }), fakeExports);
  assert.equal(resolveWasmExports("plain"), "plain");
});

test("resolveWasmExports and initializeWasmBindings accept compiled wasm modules", { concurrency: false }, () => {
  const compiledModule = new WebAssembly.Module(
    readFileSync(new URL("../pkg/par3_bg.wasm", import.meta.url)),
  );
  const wrappedResolved = resolveWasmExports({ default: compiledModule }, par3RawBindings);
  const directResolved = resolveWasmExports(compiledModule, par3RawBindings);

  try {
    assert.equal(typeof wrappedResolved.memory.grow, "function");
    assert.equal(typeof wrappedResolved.__wbindgen_start, "function");
    assert.equal(typeof directResolved.alloc_shard_arena, "function");

    const initialized = initializeWasmBindings({ default: compiledModule }, par3RawBindings);

    assert.equal(typeof initialized.shard_arena_ptr, "function");
    assert.equal(typeof par3RawBindings.alloc_shard_arena, "function");
  } finally {
    par3RawBindings.__wbg_set_wasm(wasmModule);
  }
});
