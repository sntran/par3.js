...
function isClosedPartWriterError(error) {
  ...
  return code === "ERR_INVALID_STATE"
    || (error instanceof TypeError && /closed|closing|Invalid state/u.test(message));
}
...
function createMultipartDecoderTransformer({ boundary, maxPartHeaderBytes }) {
  ...
  const closeCurrentPart = async () => {
    ...
    try {
      ...
      if (!isClosedPartWriterError(error)) {
        throw error;
      }
    } finally {
      writer.releaseLock();
  ...
  const writeCurrentPartChunk = async (chunk) => {
    await currentPartWriter.write(Uint8Array.from(chunk));
  };
